<?php 
	...
	$postData = file_get_contents("php://input");
	 ...
	 $from='info@careandcure.com' ;   
		...
		if($name !="" &&  $number !="" &&  $email !="" &&  $date !="" &&  $message !=""  )   
		{
		//	$datea=explode('/',$date);
			
		//	$date=$datea[2].'-'.$datea[0].'-'.$datea[1];
			...
			if($user_id=='')
			{
				...
				if ( $user_id=='' ) 
				{
					...
					if ( $user_id=='' ) 
					{
						...
						<td style="border: 1px solid #eee; padding: 10px;">'. $_POST['clientEmail'] .'</td>
			...
			$sqlquery="INSERT INTO appointments(user_id,name,phone_number,emailid,appointment_date,question,created_datetime)
			...
			<tr> 
			<td style="width: 200px; font-weight: bold; border: 1px solid #eee; padding: 10px;">Date</td> 
			<td style="border: 1px solid #eee; padding: 10px;">'. $date .'</td> 
			</tr>
			...
			$subject='Careandcure - Appointment info';
			...
			$to=$email;
			$subject="Request for appointment  has been received and it will be update soon";
